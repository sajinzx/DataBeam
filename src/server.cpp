...
uint16_t load_checkpoint(const string& target_filename) {
    ...
    if (f_pos != string::npos && s_pos != string::npos) {
        ...
        while(s_start < content.length() && isspace(content[s_start])) s_start++;
        size_t s_end = content.find(",", s_start);
        if (s_end == string::npos) s_end = content.find("}", s_start);
        while(s_end > s_start && isspace(content[s_end-1])) s_end--;
        
        uint16_t seq = stoi(content.substr(s_start, s_end - s_start));
        
        if (saved_file == target_filename) return seq;
    }
    return 1;
}
            ...
            if (crc_valid) {
                ...
                if (pkt.seq_num == expected_seq_num) {
                    ...
                    if (pkt.type == 3) {
                        ...
                        if (remaining < decompressed_len) bytes_to_write = remaining;
                    }
                    ...
                    while (receive_buffer.find(expected_seq_num) != receive_buffer.end()) {
                        ...
                        if (buffered_pkt.type == 3) {
                            ...
                            if (remaining < buf_decomp_len) write_len = remaining;
                        }
